...
for P2 in [300]:
	for M2 in [1]:
		for omega2 in omega1grid:
			for e2 in e1grid:
				e1 = 0.52087
				omega1 = 1.2468029277718442
				...
				o2,i2,O2=GeneratePlanetaryParameters(89.8584/180*np.pi,0,omega1)
				sim.add(m=0.00123006053303,a=1.06484331396,e=e2,omega=omega2,M=np.random.rand()*2*np.pi,inc=i2,Omega=O2)
				...
				sim.add(m=1/1047*0.220,P=P2/365.2422,e=e2,omega=omega2,M=np.random.rand()*np.pi*2,inc=np.pi/2)
				R1=0.004649130334214449
				R2=MassToRadius(M2)*0.004649130334214449
				R=R1+R2
				sim.move_to_com()
